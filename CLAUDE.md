...
- 토스 포인트 프로모션(2026-09-08 시작, 10/31 종료): 산행 시작 3P, 첫 산행 30P(혜택탭), 정상 인증 10P. `_shared/toss.ts`가 insert 성공 뒤 서버 지급. 코드는 시크릿 `PROMO_*_CODE`, 비우면 지급 없음. 클라이언트에서 직접 지급하지 않는다. 예산은 줄일 수 없고 `addAmount`로 증액만 된다.
